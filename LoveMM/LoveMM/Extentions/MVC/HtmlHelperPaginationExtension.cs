...
namespace LoveMM.Extentions.MVC
{
    ...
    public static class HtmlHelperPaginationExtension
    {
        ...
        private static MvcHtmlString PagingButton(this HtmlHelper html, int PageIndex, int PageSize, long TotalRecords, bool enableAjax, string updateTargetId, PaginationMode paginationMode = PaginationMode.NumericNextPreviousFirstLast, int numericPagingButtonCount = 5, string ajaxUrl = null)
        {
            ...
            string pagingContainer = "<div class=\"page\"";
            pagingContainer += enableAjax ? " plugin=\"ajaxPagingButton\" data=\"" + HttpUtility.HtmlEncode(Json.Encode(new { updateTargetId = updateTargetId })) + "\"" : ">";
            pagingContainer += "<ul>";
            ...
            if (showFirst)
            {
                ...
                {
                    pagingButtonsHtml.AppendLine();
                    pagingButtonsHtml.AppendFormat("<li><a href=\"javascript:;\" class=\"homePage\">{0}</a></li>", "首页");
                }
            }
            ...
            if (showPrevious)
            {
                ...
                if (PageIndex == 1 || totalPages <= 0)
                    pagingButtonsHtml.AppendFormat("<li><a href=\"javascript:;\" class=\"PagePrev\">{0}</a></li>", "上一页");
                else
            ...
            if (showNumeric)
            {
                ...
                for (int i = startNumericPageIndex; i <= lastNumericPageIndex; i++)
                {
                    pagingButtonsHtml.AppendLine();
                    if (PageIndex == i)
                        pagingButtonsHtml.AppendFormat("<li class=\"thisclass\">{0}</li>", i);
                    else
            ...
            if (showNext)
            {
                ...
                if (PageIndex == totalPages || totalPages <= 0)
                    pagingButtonsHtml.AppendFormat("<li><a href=\"javascript:;\" class=\"PageNext\">{0}</a></li>", "下一页");
                else
            ...
            if (showLast)
            {
                ...
                {
                    pagingButtonsHtml.AppendLine();
                    pagingButtonsHtml.AppendFormat("<li><a href=\"javascript:;\" class=\"lastPage\">{0}</a></li>", "末页");
                }
            }
            pagingButtonsHtml.AppendLine();
            ...
            //pagingButtonsHtml.AppendLine();
            pagingButtonsHtml.AppendFormat("<li><span class=\"pageinfo\">共<strong>{0}</strong>条，共<strong>{1}</strong>页</span></li>", TotalRecords, totalPages);
            pagingButtonsHtml.Append("</ul></div>");
